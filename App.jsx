...
function MatchQ({ item, locked, state, setState }) {
  ...
  const { done, sel, wrong, miss } = state;
  const missRef = useRef(null);
  useEffect(() => () => clearTimeout(missRef.current), []);

  const tapLeft = (i) => {
    if (locked || done.includes(i)) return;
    setState({ ...state, sel: sel === i ? null : i, miss: null });
  };

  const tapRight = (i) => {
    if (locked || done.includes(i)) return;
    if (sel === null) {
      // Tapped a description first — nudge rather than doing nothing.
      setState({ ...state, miss: { side: "hint" } });
      clearTimeout(missRef.current);
      missRef.current = setTimeout(() => setState((s) => ({ ...s, miss: null })), 900);
      return;
    }
    if (sel === i) {
      setState({ ...state, done: [...done, i], sel: null, miss: null });
      return;
    }
    setState({ ...state, wrong: wrong + 1, miss: { left: sel, right: i } });
    clearTimeout(missRef.current);
    missRef.current = setTimeout(
      () => setState((s) => ({ ...s, sel: null, miss: null })),
      520
    );
  };

  const reveal = () => setState({ ...state, done: item.pairs.map((_, i) => i), sel: null, miss: null });

  const cell = (opts) => {
    const { active, ok, bad } = opts;
    return {
      fontFamily: FONT_UI, fontSize: 14, lineHeight: 1.35, padding: "12px",
      borderRadius: 12, cursor: locked ? "default" : "pointer",
      border: `1px solid ${bad ? C.no : ok ? C.ok : active ? C.glow : C.line}`,
      background: bad ? "rgba(255,158,125,.16)" : ok ? "rgba(79,216,196,.14)" : active ? C.raise : C.shelf,
      color: bad ? C.no : ok ? C.ok : C.foam,
      display: "flex", alignItems: "center", height: "100%",
      transition: "background .12s, border-color .12s",
    };
  };

  const allDone = done.length === item.pairs.length;

  return (
    <>
      <Prompt>{item.q}</Prompt>
      {/* One grid, so each row lines up across both columns. */}
        ...
        {L.map((left, row) => {
          ...
          return (
            <React.Fragment key={row}>
              <div
                onClick={() => tapLeft(left.i)}
                style={cell({
                  active: sel === left.i,
                  ok: done.includes(left.i),
                  bad: miss?.left === left.i,
                })}
              >
                {left.t}
              </div>
              <div
                onClick={() => tapRight(right.i)}
                style={cell({
                  active: false,
                  ok: done.includes(right.i),
                  bad: miss?.right === right.i,
                })}
              >
                {right.t}
              </div>
      ...
      <p style={{ fontFamily: FONT_UI, fontSize: 14, color: miss ? C.no : C.mist, marginTop: 12, minHeight: 20 }}>
        {miss?.side === "hint"
          ? "Tap a term on the left first."
          : miss
          ? "Not a pair — try another."
          : allDone
          ? wrong === 0
            ? "All matched, no mispairs."
            : `All matched, with ${wrong} mispair${wrong > 1 ? "s" : ""}.`
          : sel !== null
          ? "Now tap its description."
          : "Tap a term, then tap its description."}
      </p>

      {!locked && !allDone && wrong >= 2 && (
        <button onClick={reveal} style={{
          fontFamily: FONT_UI, fontSize: 14, padding: "10px 14px", borderRadius: 11,
          border: `1px solid ${C.line}`, background: "transparent", color: C.mist, cursor: "pointer",
        }}>
          Show me the pairs
        </button>
      )}
...
export default function App() {
  ...
  function initAnswer(it) {
    ...
    if (it.type === "match") return { done: [], sel: null, wrong: 0, miss: null };
